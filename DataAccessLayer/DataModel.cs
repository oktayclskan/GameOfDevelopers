...
namespace DataAccessLayer
{
    public class DataModel
    {
        ...
        public List<Uye> UyeListele()
        {
            try
            {
                ...
                while (reader.Read())
                {
                    ...
                    u.DurumStr=reader.GetBoolean(6) ? "<label style='color:green'>Aktif</label>" : "<label style='color:gray'>Askıya Alınmış</label>";
                    uyeler.Add(u);
        ...
        public void UyeAskiyaAl(int id)
        {
            try
            {
                cmd.CommandText = "UPDATE Uyeler Set Durum = 0 WHERE ID=@id ";
                cmd.Parameters.Clear();
                cmd.Parameters.AddWithValue("@id",id);
                con.Open();
                cmd.ExecuteNonQuery();
        ...
        public void UyeBanKaldır(int id)
        {
            try
            {
                cmd.CommandText = "UPDATE Uyeler Set Durum = 1 WHERE ID=@id ";
                cmd.Parameters.Clear();
                cmd.Parameters.AddWithValue("@id", id);
                con.Open();
                cmd.ExecuteNonQuery();
